# Shared HTML report engine for TenantToolbox.
# Provides CSS + generic JS (search/filter/sort/theme/collapse) and assembles the page.
# JS conventions:
#   - A filterable/searchable element has class="item", data-search="<lowercase>", data-name="<lowercase>".
#   - Filter membership via data-f-<KEY>="1|0". A filter button/KPI has data-filter="<KEY>" ("all" = everything).
#   - Sortable table headers: <th data-sort="KEY">. Element carries data-name or data-s-<KEY>.

function TTEnc { param($s) [System.Net.WebUtility]::HtmlEncode([string]$s) }

function Get-TTExportTarget {
    # Determines format+path from a report cmdlet's -Csv/-Excel switches. Returns $null if nothing requested.
...
function Complete-TTReport {
    # Unified finish for all report cmdlets: write HTML (unless -NoHtml), optionally
    # export CSV/XLSX, open the appropriate file (unless -NoOpen).
    param(
    ...
    if (-not $NoHtml) {
        ...
        Write-Host "$Kind created: $Path" -ForegroundColor Green
        $openTarget = $Path
    }
    # -NoHtml without a data format -> at least CSV, otherwise there would be no output.
    ...
    if ($tgt -and $null -ne $Data) {
        ...
        Write-Host "Data exported: $written" -ForegroundColor Green
...
function Export-TTData {
    # Writes flat objects as CSV or (if ImportExcel is available) as XLSX. Falls back to CSV otherwise.
    param(
    ...
    if ($Format -eq 'Excel') {
        ...
        Write-Warning "Module 'ImportExcel' is missing (Install-Module ImportExcel -Scope CurrentUser) - writing CSV instead of XLSX."
...
function New-TTKpis {
    # $Kpis: array of @{ n=number; l=label; kind='ok|bad|adm|report|off|on|block|mfa'; filter='KEY' }
    param([object[]]$Kpis)
...
function New-TTToolbar {
    # $Filters: array of @{ label='...'; key='KEY' }
    param([object[]]$Filters, [string]$SearchPlaceholder = 'Search ...', [switch]$WithToggleAll)
    ...
    $toggle = if ($WithToggleAll) { '<button class="tbtn" id="toggleAll">Collapse all</button>' } else { '' }
    @"
    <div class="toolbar">
      <div class="search">
...
$ToolbarHtml
$BodyHtml
    <div class="footer"><div class="logo">$bi</div><div>Generated with <b>$bn</b> &middot; $bt</div></div>
  </div>
...
(function(){
  ...
  function apply(){
    ...
    if(count)count.textContent=vis+' of '+items.length+' shown';
  ...
  if(ta){var col=false;ta.addEventListener('click',function(){col=!col;document.querySelectorAll('.card').forEach(function(c){c.classList.toggle('collapsed',col);});ta.textContent=col?'Expand all':'Collapse all';});}
  ...
  function applyTheme(){var t=themes[ti];
    ...
    var tb=document.getElementById('theme');if(tb)tb.innerHTML='&#9681; '+(t==='auto'?'Auto':(t==='light'?'Light':'Dark'));
