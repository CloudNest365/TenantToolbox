function Export-M365ConditionalAccessReport {
    <#
    .SYNOPSIS
        Generates a modern, self-contained HTML report of all Conditional Access policies.
    .DESCRIPTION
        Reads all CA policies via Microsoft Graph, resolves GUIDs to readable names
        (users, groups, roles, apps, named locations) and renders a nice HTML report:
        a KPI overview, one card per policy with assignments and controls, plus
        automatically computed impact/hint pills (e.g. "Block access", "Enforces MFA",
        "Applies to ALL users", "N exclusions", "Report-only").

        The report is a single HTML file (inline CSS, light/dark, no external dependencies).
        Read-only - changes nothing.
    .PARAMETER Path
        Target path of the HTML file. Default: .\ConditionalAccess-Report.html
    .PARAMETER PassThru
        Also emit the structured policy objects on the pipeline.
    .PARAMETER NoOpen
        Do NOT open the report in the default browser after generating it.
    .EXAMPLE
    ...
    param(
        ...
        # Branding in the report header/footer. For CloudNest365 simply: -BrandName 'CloudNest365'
        [string]$BrandName = 'TenantToolbox',
        [string]$BrandTagline = 'M365 Tenant Administration',

        # Additionally export the raw data as CSV or XLSX (XLSX needs the ImportExcel module).
        [switch]$Csv,
        [switch]$Excel,
        [string]$DataPath,
        [switch]$NoHtml,
    ...
    Assert-TTGraph
    Write-TTLog -Level INFO -Message "Reading Conditional Access policies ..."
    ...
    function Resolve-Users { param($Ids)
        foreach ($id in @($Ids)) {
            ...
            switch ($id) {
                'All'                    { 'All users'; break }
                'None'                   { 'None'; break }
                'GuestsOrExternalUsers'  { 'Guests / external users'; break }
    ...
    function Resolve-Apps { param($Ids)
        foreach ($id in @($Ids)) {
            if ([string]::IsNullOrWhiteSpace($id)) { continue }
            switch ($id) {
                'All'                                    { 'All cloud apps'; break }
                'None'                                   { 'None'; break }
                'Office365'                              { 'Office 365'; break }
                'MicrosoftAdminPortals'                  { 'Microsoft admin portals'; break }
    ...
    function Resolve-Locations { param($Ids)
        foreach ($id in @($Ids)) {
            if ([string]::IsNullOrWhiteSpace($id)) { continue }
            switch ($id) {
                'All'         { 'All locations'; break }
                'AllTrusted'  { 'All trusted locations'; break }
                default { if ($locMap.ContainsKey($id)) { $locMap[$id] } else { $id } }
            }
        }
    }

    $stateLabel = @{ 'enabled' = 'Enabled'; 'disabled' = 'Off'; 'enabledForReportingButNotEnforced' = 'Report-only' }
    $ctrlLabel  = @{
        'mfa' = 'MFA'; 'block' = 'Block access'; 'compliantDevice' = 'Compliant device'
        'domainJoinedDevice' = 'Domain-joined'; 'approvedApplication' = 'Approved app'
        'compliantApplication' = 'App protection policy'; 'passwordChange' = 'Password change'
    }

    # --- Process policies into objects --------------------------------------
    $report = foreach ($p in $policies) {
        ...
        # Impact / hints
        $insights = @()
        if ($p.GrantControls.BuiltInControls -contains 'block') { $insights += @{ k = 'block'; t = 'Blocks access' } }
        if ($p.GrantControls.BuiltInControls -contains 'mfa')   { $insights += @{ k = 'good';  t = 'Enforces MFA' } }
        if ($c.Users.IncludeUsers -contains 'All')              { $insights += @{ k = 'warn';  t = 'Applies to ALL users' } }
        $exCount = @($c.Users.ExcludeUsers).Count + @($c.Users.ExcludeGroups).Count + @($c.Users.ExcludeRoles).Count
        if ($exCount -gt 0)                                     { $insights += @{ k = 'warn';  t = "$exCount exclusion(s)" } }
        if ($p.State -eq 'enabledForReportingButNotEnforced')   { $insights += @{ k = 'report'; t = 'Not enforced (report-only)' } }
        if ($p.State -eq 'disabled')                            { $insights += @{ k = 'off';   t = 'Disabled' } }
        if (@($c.SignInRiskLevels).Count -or @($c.UserRiskLevels).Count) { $insights += @{ k = 'info'; t = 'Risk-based' } }
        $legacy = @($c.ClientAppTypes | Where-Object { $_ -in 'exchangeActiveSync', 'other' })
        if ($legacy.Count -and ($p.GrantControls.BuiltInControls -contains 'block')) { $insights += @{ k = 'good'; t = 'Blocks legacy auth' } }
        ...
        [pscustomobject]@{
            ...
            Grants        = $grants
            Session       = @(
                if ($p.SessionControls.SignInFrequency.IsEnabled) { 'Sign-in frequency' }
                if ($p.SessionControls.PersistentBrowser.IsEnabled) { 'Persistent browser' }
                if ($p.SessionControls.ApplicationEnforcedRestrictions.IsEnabled) { 'App-enforced restrictions' }
    ...
    $blkCount = @($report | Where-Object { $_.Grants -contains 'Block access' }).Count
    ...
    $cards = foreach ($r in $report) {
        ...
        $grantText = if ($r.Grants) { (@($r.Grants) -join " <span class='op'>$($r.GrantOperator)</span> ") } else { "<span class='muted'>No grant controls</span>" }

        $mfaFlag   = if ($r.Grants -contains 'MFA') { '1' } else { '0' }
        $blockFlag = if ($r.Grants -contains 'Block access') { '1' } else { '0' }
    ...
    <article class="card" data-state="$stateKind" data-mfa="$mfaFlag" data-block="$blockFlag" data-name="$nameAttr" data-modified="$modAttr" data-search="$searchAttr">
      ...
      <div class="grid">
        <section>
          ...
          <div class="row"><span class="lbl">Included</span><div class="vals">$(New-Chips $r.IncludeUsers 'chip chip-inc')</div></div>
          <div class="row"><span class="lbl">Excluded</span><div class="vals">$(New-Chips $r.ExcludeUsers 'chip chip-exc')</div></div>
          <div class="block-title">What (apps)</div>
          <div class="row"><span class="lbl">Included</span><div class="vals">$(New-Chips $r.IncludeApps 'chip chip-inc')</div></div>
          <div class="row"><span class="lbl">Excluded</span><div class="vals">$(New-Chips $r.ExcludeApps 'chip chip-exc')</div></div>
        </section>
        <section>
          <div class="block-title">Conditions</div>
          <div class="row"><span class="lbl">Platforms</span><div class="vals">$(New-Chips $r.Platforms)</div></div>
          <div class="row"><span class="lbl">Locations</span><div class="vals">$(New-Chips $r.Locations)</div></div>
          <div class="row"><span class="lbl">Client apps</span><div class="vals">$(New-Chips $r.ClientApps)</div></div>
          <div class="row"><span class="lbl">Sign-in risk</span><div class="vals">$(New-Chips $r.SignInRisk)</div></div>
          <div class="row"><span class="lbl">User risk</span><div class="vals">$(New-Chips $r.UserRisk)</div></div>
          <div class="block-title">Controls</div>
          <div class="row"><span class="lbl">Grant</span><div class="vals grant">$grantText</div></div>
          <div class="row"><span class="lbl">Session</span><div class="vals">$(New-Chips $r.Session)</div></div>
        </section>
      </div>
      <footer class="card-foot">Last modified: $(& $enc $r.Modified)</footer>
      </div>
    </article>
"@
    }
...
<body>
  <div class="hero">
    <div class="wrap">
      ...
      <p class="sub">Tenant $tenant &middot; generated $genAt &middot; $total policies</p>
    </div>
  </div>

  <div class="wrap">
    <div class="kpis">
      <div class="kpi active" data-filter="all"><div class="n">$total</div><div class="l">Total policies</div></div>
      <div class="kpi on" data-filter="on"><div class="n">$onCount</div><div class="l">Enabled</div></div>
      <div class="kpi report" data-filter="report"><div class="n">$repCount</div><div class="l">Report-only</div></div>
      <div class="kpi off" data-filter="off"><div class="n">$offCount</div><div class="l">Disabled</div></div>
      <div class="kpi mfa" data-filter="mfa"><div class="n">$mfaCount</div><div class="l">Enforce MFA</div></div>
      <div class="kpi block" data-filter="block"><div class="n">$blkCount</div><div class="l">Block</div></div>
    </div>
    ...
    <div class="toolbar">
      <div class="search">
        ...
        <input id="q" type="search" placeholder="Search policies (name, user, app, location ...)" autocomplete="off">
      </div>
      <div class="filters" id="filters">
        <button class="fbtn active" data-filter="all">All</button>
        <button class="fbtn" data-filter="on">Enabled</button>
        <button class="fbtn" data-filter="report">Report-only</button>
        <button class="fbtn" data-filter="off">Disabled</button>
        <button class="fbtn" data-filter="mfa">MFA</button>
        <button class="fbtn" data-filter="block">Block</button>
      </div>
      <select id="sort" title="Sorting">
        <option value="name">Name (A–Z)</option>
        <option value="name-desc">Name (Z–A)</option>
        <option value="modified-desc">Last modified</option>
        <option value="state">Status</option>
      </select>
      <button class="tbtn" id="toggleAll" title="Expand/collapse all">Collapse all</button>
      <button class="tbtn" id="theme" title="Switch theme">&#9681; Theme</button>
...
$($cards -join "`n")
    ...
    <div class="footer">
      <div class="logo">$brandInitial</div>
      <div>Generated with <b>$brandName</b> &middot; $brandTagline &middot; $genAt</div>
    </div>
  </div>
...
(function(){
  ...
  function applyTheme(){
    ...
    document.getElementById('theme').innerHTML = '&#9681; ' + (t==='auto'?'Auto':(t==='light'?'Light':'Dark'));
...
"@
    ...
    Write-TTLog -Level INFO -Message "Report created: $Path ($total policies)."
