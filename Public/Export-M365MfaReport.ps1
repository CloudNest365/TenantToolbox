function Export-M365MfaReport {
    <#
    .SYNOPSIS
        Generates a modern, interactive HTML report of the MFA status of all users.
    .DESCRIPTION
        Builds on Get-M365MfaStatus and renders a self-contained HTML page: a KPI overview
        (registered / not registered / unprotected admins ...) and a searchable, filterable,
        sortable table with status badges and registered methods. Read-only.
    .PARAMETER Path
        Target path of the HTML file. Default: .\MFA-Report.html
    .PARAMETER IncludeGuests
        Also include guest accounts (default: members only).
    .PARAMETER BrandName
        Branding in header/footer. For CloudNest365: -BrandName 'CloudNest365'
    .PARAMETER BrandTagline
        Subtitle next to the brand name.
    .PARAMETER PassThru
        Also emit the status objects on the pipeline.
    .PARAMETER NoOpen
        Do NOT open the report in the browser after generating it.
    .EXAMPLE
    ...
    $defaultLabel = @{
        'push' = 'Authenticator (Push)'; 'oath' = 'OTP code'; 'sms' = 'SMS'; 'voice' = 'Call'
        'mobilePhone' = 'SMS / Call'; 'none' = '–'; '' = '–'
    }
    ...
    $rows = foreach ($u in ($data | Sort-Object { -not $_.MfaRegistered }, DisplayName)) {
        ...
        $typLabel  = if ($typ -eq 'guest') { 'Guest' } else { 'Member' }

        $mfaBadge = if ($u.MfaRegistered) { "<span class='b b-on'>registered</span>" } else { "<span class='b b-off'>missing</span>" }
...
<body>
  <div class="hero"><div class="wrap">
    ...
    <p class="sub">Tenant $tenant &middot; generated $genAt &middot; $total users</p>
  </div></div>

  <div class="wrap">
    <div class="kpis">
      <div class="kpi active" data-filter="all"><div class="n">$total</div><div class="l">Users</div></div>
      <div class="kpi ok" data-filter="reg"><div class="n">$reg</div><div class="l">MFA registered</div></div>
      <div class="kpi bad" data-filter="unreg"><div class="n">$unreg</div><div class="l">Without MFA</div></div>
      <div class="kpi adm" data-filter="admin"><div class="n">$admins</div><div class="l">Admins</div></div>
      <div class="kpi bad" data-filter="adminbad"><div class="n">$adminBad</div><div class="l">Admins without MFA</div></div>
      <div class="kpi ok" data-filter="pwless"><div class="n">$pwless</div><div class="l">Passwordless-capable</div></div>
    </div>
    ...
    <div class="toolbar">
      <div class="search">
        ...
        <input id="q" type="search" placeholder="Search users (name, UPN, method ...)" autocomplete="off">
      </div>
      <div class="filters" id="filters">
        <button class="fbtn active" data-filter="all">All</button>
        <button class="fbtn" data-filter="reg">Registered</button>
        <button class="fbtn" data-filter="unreg">Without MFA</button>
        <button class="fbtn" data-filter="admin">Admins</button>
        <button class="fbtn" data-filter="adminbad">Admins without MFA</button>
...
$($rows -join "`n")
    ...
    <div class="footer"><div class="logo">$brandInitial</div><div>Generated with <b>$brandName</b> &middot; $brandTagline &middot; $genAt</div></div>
  </div>
...
"@
    ...
    Write-TTLog -Level INFO -Message "MFA report created: $Path ($total users, $unreg without MFA, $adminBad admins unprotected)."
    if ($adminBad -gt 0) { Write-Host "  Warning: $adminBad admin(s) without registered MFA!" -ForegroundColor Red }
